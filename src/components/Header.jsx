...
function Header(props) {
  const getLinkClasses = ({ isActive }) =>
    `${
      isActive ? "text-pink-500 border-pink-500 " : "text-white border-black"
    } px-4 py-1 border-b-2 hover:text-pink-500 `;

  return (
    <div className="flex bg-black w-full p-4 text-white items-center justify-between shadow-md">
      <div className="font-semibold">
        <Link
          to="/"
          className="hover:text-pink-600 transition-colors duration-300 font-glitch text-5xl ml-4"
        >
          React Store
        </Link>
      </div>

      <div className="flex gap-x-8 text-lg font-medium font-montserrat">
      ...
      <NavLink
        to="/cart"
        className="px-1 mr-4 relative border-2 border-transparent text-gray-800 rounded-full hover:text-gray-400 focus:outline-none focus:text-gray-500 transition duration-150 ease-in-out"
        aria-label="Cart"
      >
        <svg
        ...
        {props.cartItems.length > 0 ? (
          <span className="absolute inset-x-7 -inset-y-2 object-right-top mr-2">
